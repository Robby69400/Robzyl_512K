...
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_RU_BAND=1  ENABLE_EEPROM_512K=1    TARGET=robzyl.512k.in            && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_IN_BAND=1  ENABLE_EEPROM_512K=1    TARGET=robzyl.512k.ru            && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_BR_BAND=1  ENABLE_EEPROM_512K=1    TARGET=robzyl.512k.ru            && cp *packed.bin compiled-firmware/"
...
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_FR_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.fr.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_PL_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.pl.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_RO_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.ro.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_KO_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.ko.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_CZ_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.cz.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_TU_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.tu.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_RU_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.ru.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_IN_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.in.screenshot   && cp *packed.bin compiled-firmware/"
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_BR_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.8k.br.screenshot   && cp *packed.bin compiled-firmware/"
