...
docker build -t uvk5 .
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make clean && make -s ENABLE_FR_BAND=1  ENABLE_EEPROM_512K=1    TARGET=robzyl.512k.ko            && cp *packed.bin compiled-firmware/"
...
REM docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make -s               ENABLE_KO_BAND=1  ENABLE_SCREENSHOT=1     TARGET=robzyl.screenshot.ko      && cp *packed.bin compiled-firmware/"
