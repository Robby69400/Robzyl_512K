...
typedef struct Time {
  uint8_t hour;
  uint8_t minute;
  uint8_t second; // added seconds for local running clock
} Time;
...
/* Global running RDS time (set when group 4A parsed). After valid sync:
   gRdsTimeValid == true and gRdsLocalTime holds current time (seconds start at 0).
   The application will increment seconds between RDS updates.
*/
extern Time gRdsLocalTime;
extern bool gRdsTimeValid;
