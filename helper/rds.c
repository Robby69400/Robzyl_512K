...
/* Definicje globalne — muszą istnieć dokładnie raz w projekcie */
Time gRdsLocalTime = {0, 0, 0};
bool gRdsTimeValid = false;

/* Deklaracja flagi, którą ustawimy, aby GUI się odświeżyło.
   Flaga jest zdefiniowana w głównych plikach aplikacji (app.c). */
extern volatile bool gUpdateDisplay;
//extern volatile bool gUpdateStatus;
...
/*         // Check if date and time sent (not 0)
		...
		/* Inside the code path that handles Group 4A - Clock-time and date (near your existing
   code block for type == 4 && version == 0), after you set rds.MJD, rds.hour, rds.minute
   and rds.offset and set new_info = true; add the following lines: */

// After successful parsing of Group 4A (replace the comment/new_info assignment area)
        if (rds.MJD || rds.hour || rds.minute || rdsResponse.raw[Block_D_L]) {
          // Get offset to convert UTC to local time
          rds.offset = rdsResponse.raw[Block_D_L] & 0x1F;
          // Check if offset should be negative
          ...
          // set global running RDS time (seconds start at 0)
          gRdsLocalTime.hour = rds.hour;
          gRdsLocalTime.minute = rds.minute;
  ...
  // Dzięki temu nazwa stacji / RDS / czas od razu pojawią się na ekranie.
  if (new_info)
  {
      gUpdateDisplay = true;
      //gUpdateStatus = true;
