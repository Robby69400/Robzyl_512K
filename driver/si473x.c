...
// Dodane nagłówki do obsługi wyświetlania
#include "ui/helper.h"
...
//    const uint8_t PAGE_SIZE = SETTINGS_GetPageSize();
    ...
    // Podgląd 8 bajtów
    uint8_t preview[8];
    EEPROM_ReadBuffer(PATCH_START, preview, 8);
    
    char hexInfo[32];
    sprintf(hexInfo, "%02X%02X%02X%02X %02X%02X%02X%02X", 
        preview[0], preview[1], preview[2], preview[3],
        preview[4], preview[5], preview[6], preview[7]);

    UI_PrintStringSmall(hexInfo, 0, 127, 4, 0); 
